...
desc boe;
select * from BOE;
delete from BOE where student_id = 102;
update boe SET s_name ='Shiv' WHERE student_id=102;
...
delete from category where c_id=101;
select * from category;
create table college (C_Id int, C_Name varchar(20));
create table department(D_Id int, D_Name varchar(30), 
C_Id int not null, 
constraint C_Id foreign key(C_id) 
references college(C_Id));
create table student (S_Id int, S_Name varchar(100), 
C_Id int not null, D_Id int not null, 
constraint C_Id foreign key(C_id) 
references college(C_Id),
constraint D_Id foreign key(C_id) 
...
SELECT DEPARTMENT, COUNT(*) AS NumWorkers
FROM Worker
GROUP BY DEPARTMENT
HAVING COUNT(*) < 5;
SELECT * FROM Worker 
...
insert into student values (1001,"Amatya","ECE"),(1002,"Praveen","CSE"),(1003,"Yukesh","Mech"),(1006,'karthick','Aero'),(1007,"Mahesh","Civil");
